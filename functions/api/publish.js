...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (origin && origin !== allowedOrigin) {
      return jsonResponse({ success: false, message: "Unauthorized origin." }, 403);
    }
    ...
    if (!env.PUBLISHER_PASSWORD) {
      return jsonResponse({
        success: false,
        message: "Publisher is not configured yet. Add PUBLISHER_PASSWORD in Cloudflare."
      }, 500);
    }

    if (!publisherPassword || publisherPassword !== env.PUBLISHER_PASSWORD) {
      return jsonResponse({ success: false, message: "Incorrect publisher password." }, 401);
    }
    ...
    if (!githubToken) {
      return jsonResponse({
        success: false,
        message: "GitHub publishing is not configured yet. Add GITHUB_TOKEN in Cloudflare."
      }, 500);
    }
    ...
    // This prevents a JSON parsing error from hiding
    // the actual GitHub error message.
...
<header>
  <div class="logo">LifeKey Hub</div>
