...
function formatContent(text) {
  ...
  html = html
    ...
    .replace(
      /^> INFO: (.*)$/gm,
      "<aside><b>💡 INFO:</b> $1</aside>"
    )
    .replace(
      /^> TIP: (.*)$/gm,
      "<aside><b>✅ TIP:</b> $1</aside>"
    )
    .replace(
      /^> WARNING: (.*)$/gm,
      "<aside><b>⚠️ WARNING:</b> $1</aside>"
    )
    .replace(
      /^> (.*)$/gm,
...
async function connectCategory(category, token) {
  ...
  const file = await getFile(destination, token);
...
export async function onRequestPost({
  ...
  try {
    ...
    /*
      IMPORTANT:
      Use absolute website URLs here.
      This prevents GitHub Pages 404 errors
      caused by article folder depth.
    */
