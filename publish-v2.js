...
async function get(path,t){
  ...
  if(!r.ok){
    throw Error(
      `GitHub GET ${path}: ${r.status} ${await r.text()}`
    );
  }
...
async function put(path,text,msg,t,sha){
  ...
  if(!r.ok){
    throw Error(
      `GitHub PUT ${path}: ${r.status} ${await r.text()}`
    );
  }
...
export async function onRequestPost({request,env}){
  try{
    ...
    if(
      !d.password ||
      d.password!==env.PUBLISHER_PASSWORD
    ){
      return new Response(
        JSON.stringify({
    ...
    /*
      ...
      Articles can now live several folders deep,
      so we always use the full GitHub Pages URL.
    */

    const destination=DEST[d.category];
    ...
    const rec={
      ...
      seoDescription:
        d.seoDescription||d.description,
      category:d.category,
